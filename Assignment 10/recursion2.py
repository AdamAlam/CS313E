...
def groupSum(start, nums, target):   # start = index in the array
    if start >= len(nums):
        return target == 0
    else:
    ...
    if start >= len(nums):
        ...
        while i < len(nums)-1 and j != 0:
            if nums[i] == nums[i+1]:
                ...
                j = 0
        return groupSumClump(start + counter, nums, target - sum1) or groupSumClump(start + counter, nums, target)
    else:
...
def splitOdd10Helper(nums, i , g1, g2):
    n = len(nums)
    if i == n:
        return ((g1 % 2 == 1 and g2 % 10 == 0) or (g2 % 2 == 1 and g1 % 10 == 0))
    if (splitOdd10Helper(nums, i+1, g1+nums[i], g2)):
        return True
    return splitOdd10Helper(nums, i+1, g1, g2+nums[i])
...
def split53Helper(nums, i, test):
    n = len(nums)
    if i == n:
        return (test==0)
    if nums[i] % 5 == 0:
        return split53Helper(nums, i+1, test+nums[i])
    if nums[i] % 3 == 0:
        return split53Helper(nums, i+1, test-nums[i])
    if split53Helper(nums, i+1, test+nums[i]):
        return True
    return split53Helper(nums, i+1, test-nums[i])
